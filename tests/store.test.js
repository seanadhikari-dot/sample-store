...
test('records a non-PII profiling lifecycle',()=>{const c=addToCart(undefined,'mouse-wireless',1),x=createCheckout(c.id);assert.equal(x.profiling.status,'not_started');assert.equal(recordProfiling(x.id,'started').profiling.status,'started');assert.equal(recordProfiling(x.id,'complete').profiling.status,'complete')});
=======
